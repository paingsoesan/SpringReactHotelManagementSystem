...
export default class ApiService {
    ...
    static getHeader() {
        ...
        return {
            Authorization: `Bearer ${token}`, //Bearer mean Outh2 token
    ...
    static async getUserProfile() {
        const response = await axios.get(`http://localhost:8080/users/get-logged-in-profile-info`, {
            headers: this.getHeader()
        })
        return response.data
    }
    ...
    static async getUserBookings(userId) {
        const response = await axios.get(`http://localhost:8080/users/get-user-bookings/${userId}`, {
            headers: this.getHeader()
        })
        return response.data
    }
    ...
    static async addRoom(formData) {
        const result = await axios.post(`http://localhost:8080/rooms/add`, formData, {
            headers: {
                ...this.getHeader(),//To ensure that the request is made by an authenticated user, the Authorization header containing the user's token is included.
    ...
    static async getAllAvailableRooms() {
        const result = await axios.get(`http://localhost:8080/rooms/all-available-rooms`)
        return result.data
    }
    ...
    static async getAllRooms() {
        const result = await axios.get(`http://localhost:8080/rooms/all`)
        return result.data
    }
    /* This funcction gets a room by the id */
    static async getRoomById(roomId) {
        const result = await axios.get(`http://localhost:8080/rooms/room-by-id/${roomId}`)
        return result.data
    }

    /* This  deletes a room by the Id */
    static async deleteRoom(roomId) {
        const result = await axios.delete(`http://localhost:8080/rooms/delete/${roomId}`, {
            headers: this.getHeader()
        })
        return result.data
    }
    ...
    static async updateRoom(roomId, formData) {
        const result = await axios.put(`http://localhost:8080/rooms/update/${roomId}`, formData, {
            headers: {
                ...this.getHeader(),
                'Content-Type': 'multipart/form-data'
            }
